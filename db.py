...
class ObjectDB:
    ...
    # create by zhoufang 20221203 改进加速版本 
    def makedirs_speed(path, exist_ok: bool = False, mode: int = None) -> None:
        if mode is None:
    ...
    def add_speed( # 改进DVC版本，复写add函数，create by zhoufang 20221205
        self,
        path: "AnyFSPath",
        fs: "FileSystem",
        oid: str,
        ...
        **kwargs,
        ...
        # print("success call in add_zfang(), core transfer step: add_zfang call copyfile")
        from dvc_objects.fs import generic
        from dvc_objects.fs.callbacks import Callback
        ...
        with Callback.as_tqdm_callback(
            ...
            parent = self._parent(cache_path)
            self.makedirs_speed(parent, True)
            copyfile(path, cache_path)
    # creat by zhoufang end
